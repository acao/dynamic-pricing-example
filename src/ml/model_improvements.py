...
class EnhancedInsurancePricingModel:
    ...
    def _create_model(
        self,
    ) -> Union[DecisionTreeRegressor, RandomForestRegressor, GradientBoostingRegressor]:
        ...
        try:
            ...
            if self.model_type == "decision_tree":
                ...
                self.explainer = shap.KernelExplainer(
                    self.model.predict, self.X_train_sample
                )
...
class ModelEnsemble:
    ...
    def predict_with_explanations(
        ...
        for i in range(len(policies)):
            policy_explanations = [
                model_explanations[i]
                for model_explanations, model in zip(explanations, self.models)
