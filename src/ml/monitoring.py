...
class ModelMonitor:
    ...
    def __init__(
        ...
        self.snowflake_config = snowflake_config or self._get_snowflake_config_from_env()
        self.conn = None
    ...
    def calculate_performance_metrics(self) -> Dict:
        ...
        try:
            ...
            for row in cursor:
                policy_id, input_features, base_premium, final_premium, adjustments = row
            ...
            if len(actual_premiums) > 0:
                ...
                mape = np.mean(np.abs((np.array(actual_premiums) - np.array(predicted_premiums)) / np.array(actual_premiums))) * 100
        ...
        try:
            ...
            if len(features_list) > 0:
                ...
                for column in df.columns:
                    ...
                    range_diff = ((cur_max - cur_min) - (ref_max - ref_min)) / (ref_max - ref_min) if (ref_max - ref_min) != 0 else 0
                ...
                self.feature_drift_metrics = {
                    ...
                    "sample_size": len(df),
                    "reference_period": {
                        "start": min(reference_df['timestamp']).isoformat() if len(reference_df) > 0 else None,
                        "end": max(reference_df['timestamp']).isoformat() if len(reference_df) > 0 else None,
                    },
                    "current_period": {
                        "start": min(current_df['timestamp']).isoformat() if len(current_df) > 0 else None,
                        "end": max(current_df['timestamp']).isoformat() if len(current_df) > 0 else None,
                    },
                    "timestamp": datetime.now().isoformat(),
                }
    ...
    def log_metrics_to_snowflake(self) -> bool:
        ...
        try:
            ...
            if self.prediction_drift_metrics:
                prediction_drift_metrics_json = json.dumps(self.prediction_drift_metrics)
    ...
    def check_model_health(self) -> Dict:
        ...
        if self.performance_metrics:
            if self.performance_metrics.get("mape", 0) > performance_thresholds["mape"]:
                ...
                performance_issues.append(f"MAPE is {self.performance_metrics['mape']:.2f}%, which is above the threshold of {performance_thresholds['mape']}%")
            
            if self.performance_metrics.get("r2", 1) < performance_thresholds["r2"]:
                performance_status = "degraded"
                performance_issues.append(f"R-squared is {self.performance_metrics['r2']:.2f}, which is below the threshold of {performance_thresholds['r2']}")
            
            if abs(self.performance_metrics.get("bias", 0)) > performance_thresholds["bias"]:
                performance_status = "degraded"
                performance_issues.append(f"Bias is ${abs(self.performance_metrics['bias']):.2f}, which is above the threshold of ${performance_thresholds['bias']}")
        ...
        if self.feature_drift_metrics and "metrics" in self.feature_drift_metrics:
            for feature, metrics in self.feature_drift_metrics["metrics"].items():
                if metrics.get("psi", 0) > drift_thresholds["psi"]:
                    ...
                    feature_drift_issues.append(f"Feature '{feature}' has PSI of {metrics['psi']:.2f}, which is above the threshold of {drift_thresholds['psi']}")
                
                if abs(metrics.get("mean_diff", 0)) > drift_thresholds["mean_diff"]:
                    feature_drift_status = "degraded"
                    feature_drift_issues.append(f"Feature '{feature}' has mean difference of {abs(metrics['mean_diff']):.2f}, which is above the threshold of {drift_thresholds['mean_diff']}")
        ...
        if self.prediction_drift_metrics:
            if self.prediction_drift_metrics.get("psi", 0) > drift_thresholds["psi"]:
                ...
                prediction_drift_issues.append(f"Prediction PSI is {self.prediction_drift_metrics['psi']:.2f}, which is above the threshold of {drift_thresholds['psi']}")
            
            if abs(self.prediction_drift_metrics.get("mean_diff", 0)) > drift_thresholds["mean_diff"]:
                prediction_drift_status = "degraded"
                prediction_drift_issues.append(f"Prediction mean difference is {abs(self.prediction_drift_metrics['mean_diff']):.2f}, which is above the threshold of {drift_thresholds['mean_diff']}")
        
        # Determine overall status
        if performance_status == "degraded" or feature_drift_status == "degraded" or prediction_drift_status == "degraded":
...
class ModelDeploymentManager:
    ...
    def get_active_model(self) -> Optional[ModelVersion]:
        ...
        try:
            ...
            if row:
                model_id, model_name, model_version, model_path, metrics, created_at = row
    ...
    def get_model_versions(self, limit: int = 10) -> List[ModelVersion]:
        ...
        try:
            ...
            for row in cursor:
                model_id, model_name, model_version, model_path, is_active, metrics, created_at = row
