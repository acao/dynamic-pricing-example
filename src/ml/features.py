...
from src.ml.models import (
    Driver, DrivingHistory, Location, Policy, Vehicle, VehicleUse
)
...
class DriverFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def __init__(self):
        ...
        self.marital_status_encoder = OneHotEncoder(sparse_output=False, handle_unknown='ignore')
    ...
    def fit(self, X: List[Driver], y=None):
        ...
        marital_statuses = [driver.marital_status.value if driver.marital_status else 'unknown' for driver in X]
    ...
    def transform(self, X: List[Driver]) -> pd.DataFrame:
        ...
        for driver in X:
            ...
            age_band = pd.cut(
                [driver.age], bins=self.age_bands, labels=self.age_band_labels, right=False
            )[0]
    ...
    def transform(self, X: List[Vehicle]) -> pd.DataFrame:
        ...
        for vehicle in X:
            ...
            for vc in vehicle_classes:
                vehicle_features[f'vehicle_class_{vc}'] = 1 if vehicle_class == vc else 0
...
class DrivingHistoryFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def __init__(self):
        self.incident_type_encoder = OneHotEncoder(sparse_output=False, handle_unknown='ignore')
        self.severity_encoder = OneHotEncoder(sparse_output=False, handle_unknown='ignore')
    ...
    def transform(self, X: List[DrivingHistory]) -> pd.DataFrame:
        ...
        if X:
            ...
            years_since_last_incident = (today.year - most_recent_date.year) - (
                (today.month, today.day) < (most_recent_date.month, most_recent_date.day)
            )
        else:
            years_since_last_incident = 10  # Assume 10 years if no incidents
        ...
        incident_severity_score = sum(
            severity_weights[h.severity.value] for h in X
        )
...
class LocationFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: List[Location]) -> pd.DataFrame:
        ...
        for location in X:
            ...
            territory_risks = self.territory_risk_map.get(territory, self.territory_risk_map['default'])
        ...
        for policy in X:
            ...
            for col in driver_features.columns:
                if driver_features[col].dtype in [np.float64, np.int64]:
                    ...
                    if len(mode_values) > 0:
                        ...
                        driver_agg[f'driver_{col}_mode'] = driver_features[col].iloc[0] if len(driver_features) > 0 else 0
            ...
            for col in vehicle_features.columns:
                if vehicle_features[col].dtype in [np.float64, np.int64]:
                    ...
                    if len(mode_values) > 0:
                        ...
                        # Fallback to first value if no mode
                        vehicle_agg[f'vehicle_{col}_mode'] = vehicle_features[col].iloc[0] if len(vehicle_features) > 0 else 0
            ...
            for col in location_features.columns:
                if location_features[col].dtype in [np.float64, np.int64]:
                    ...
                    if len(mode_values) > 0:
                        ...
                        # Fallback to first value if no mode
                        location_agg[f'location_{col}_mode'] = location_features[col].iloc[0] if len(location_features) > 0 else 0
            
            # Combine all features
            policy_features = {
                'policy_duration_days': (policy.expiration_date - policy.effective_date).days,
                'num_drivers': len(policy.drivers),
                'num_vehicles': len(policy.vehicles),
                'num_locations': len(policy.locations),
                'vehicle_per_driver_ratio': len(policy.vehicles) / max(1, len(policy.drivers)),
                **driver_agg,
                **vehicle_agg,
            ...
            if policy.pricing_factors:
                if policy.pricing_factors.credit_score:
                    policy_features['credit_score'] = policy.pricing_factors.credit_score
                if policy.pricing_factors.insurance_score:
                    policy_features['insurance_score'] = policy.pricing_factors.insurance_score
